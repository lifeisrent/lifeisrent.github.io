...
서론: 머신비전 → 반도체 검사, 자율주행에 쓰임

목표: 반도체 웨이퍼 표면검사에 쓰이는 이미지 처리 알고리즘을 연구

데이터: DAGM 2007 – 난이도 낮고, 인공 생성된 유사 웨이퍼 이미지
...
- 학습 전에 이미지 크기 정규화, 밝기 보정, 노이즈 제거 등의 전처리를 수행한다.
- 현장 적용 가능성을 고려하여 연산량이 적은 기법 위주로 설계해야 한다.

**고려 포인트**
- 실시간 처리가 가능한 연산만 사용할 것
- 알고리즘의 일관성 유지와 품질 개선 효과 비교
